...
估算規則：
1. 不要直接憑菜名吐單點熱量。先拆：主食、肉類/蛋/豆製品、蔬菜、湯、醬汁、勾芡、油脂、飲料、小菜。
2. 每個主要組成先估份量範圍，再估熱量範圍，最後加總。
3. 外食看不出重量、油、糖、勾芡時，不能採過度樂觀的低值。
4. 炒飯、燴飯、咖哩飯、滷肉飯、鍋燒、濃醬、炸物、牛三寶、五花肉、加工肉要把烹調油/醬汁/較高脂肪的不確定性算進去。
5. 使用者若輸入克數、包裝營養標示或明確份量，優先使用文字縮小範圍。
6. 多張照片要先去重，不能同一盤算兩次。
7. 不要假裝照片可以精準到個位數。中心值以約 10 kcal / 5g 粒度合理。
8. 已吃完且飽足度>=7，不叫使用者立刻補吃；把調整放到下一餐。
9. 下一餐建議不得超過今天真正剩餘額度。
...
"estimate_note":"照片估算限制"
    ...
    data["next_meal"]=next_meal_guidance(
        remaining_after,
        meal_type=meal_type,
        timing=timing,
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.7.2","database":"postgres" if USE_POSTGRES else "sqlite"}
