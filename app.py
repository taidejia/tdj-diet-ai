...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    """Client-facing next-meal guidance: concrete food/portion language, no P/C/F shorthand."""
    try:
    ...
    if timing=="after" and fullness is not None and fullness>=7:
        parts.append("這餐吃完已經有飽足感，現在先不用再加餐。")
    elif timing=="after":
        parts.append("這餐已經吃完，下一次真的餓了再吃就可以。")
    else:
        parts.append("這餐如果還沒吃，可以直接照下面的方向調整。")

    if protein_need and veg_need:
        parts.append("下一餐先抓一份足量蛋白質＋1–2拳蔬菜。")
        parts.append("肉類或魚類可抓約1–1.5個掌心；也可以用豆腐搭配蛋或無糖豆漿。")
    elif protein_need:
        parts.append("下一餐優先把蛋白質顧好：肉類或魚類約1–1.5個掌心，也可以用豆腐搭配蛋或無糖豆漿。")
    elif veg_need:
        parts.append("下一餐先補1–2拳蔬菜，例如燙青菜、炒青菜或蔬菜湯。")
    else:
        parts.append("下一餐照正常份量、依飢餓感安排即可。")

    if carbs_high:
        parts.append("今天主食已經較多，下一餐主食份量先縮小。")
    else:
        parts.append("主食看餓不餓再加，不需要為了達到每日數字刻意多吃。")

    if fat_high:
        parts.append("今天油脂已接近建議量，下一餐盡量選清蒸、滷、烤、燙；炸物、肥肉、濃醬先跳過，堅果先少量。")

    if 0 < cal < 300:
        parts.append(f"今天大約只剩 {cal} kcal 的熱量參考空間，若還會吃，份量抓小份即可。")

    parts.append("上面的剩餘數字只是參考，不代表一定要全部吃完。")
    return "".join(parts)
...
16. good 只有在真的有值得肯定的飲食行為時才寫，而且要講具體食物與具體原因。不要因為有兩三種肉就說「營養層次更完整」；若只是一般搭配或沒有明顯優點，good 請輸出空字串。
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.7.9","database":"postgres" if USE_POSTGRES else "sqlite"}
