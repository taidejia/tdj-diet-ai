...
def meal():
    ...
    if request.method=="POST":
        ...
        if analysis:
            ...
            progress_text="\n".join([f"{x.get('label')}：{x.get('current'):g}/{x.get('target'):g}{x.get('unit')}｜{x.get('status')}｜{x.get('message')}" for x in dp])
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.7.4","database":"postgres" if USE_POSTGRES else "sqlite"}
