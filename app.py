    ...
    if not p.get("custom_water_ml") and p.get("weight"):
        ...
        if expected_water and round(float(p.get("daily_water_ml") or 0)) != round(expected_water):
            ...
            c.execute("UPDATE profiles SET daily_water_ml=?, updated_at=? WHERE user_key=?",
                      (expected_water,datetime.now().isoformat(timespec="seconds"),user_key()))
            c.commit(); c.close()
...
def home():
    ...
    return render_template("home.html",profile=p,line_user=current_line_user(),member=membership(),active=active,totals=totals,progress=progress,remaining=rem)
...
def daily_nutrition_progress(p, totals, meal_type=None):
    ...
    if protein_goal<=0:
        ...
        protein_note=f"今天蛋白質約 {protein_now:.0f}g，已接近或達到建議量，不需要為了湊數字額外多吃。"
        ...
        protein_status="目前偏少"
        protein_note=f"今天累積約 {protein_now:.0f}g，進度偏慢；下一個正餐優先安排一份明顯蛋白質主菜。"
    else:
        protein_status="接近目標"
    ...
    if carbs_goal<=0:
        ...
        carbs_status="尚有配置空間"
        carbs_note=f"今天碳水約 {carbs_now:.0f}g，約還有 {carb_gap:.0f}g 配置空間；依飢餓感安排即可，不用硬補滿。"
    ...
    if fat_goal<=0:
        ...
        fat_status="目前可接受"
        fat_note=f"今天脂肪約 {fat_now:.0f}g，目前仍在可接受範圍；不需要為了吃滿數字特別補油脂。"
...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    """Concrete coaching plan based on whole-day progress and true remaining ceilings."""
    ...
    protein_need=protein_status in ("目前偏少","今日明顯偏少","接近目標")
    ...
    if timing=="after" and fullness is not None and fullness>=7:
        lead="這餐吃完已經有飽足感，現在先不用再補。"
    elif timing=="after":
        lead="這餐已經吃完，先不用為了補數字立刻再吃。"
    else:
        lead="如果這餐還沒吃，可以直接把下面的重點放進這一餐。"

    priorities=[]
    if protein_need: priorities.append("一份蛋白質主菜")
    if veg_need: priorities.append("1–2 拳蔬菜")
    if priorities:
        action="下一次進食優先安排"+"＋".join(priorities)+"。"
    else:
        action="下一次進食照正常份量、依飢餓感安排即可。"

    examples=[]
    if protein_need:
        examples.append("蛋白質可選魚、雞肉、瘦肉、豆腐或蛋")
    if veg_need:
        examples.append("蔬菜可選燙青菜、炒青菜或蔬菜湯")
    example_text=("例如"+"；".join(examples)+"。") if examples else ""

    starch=""
    if carbs_high:
        starch="今天主食配置已偏高，下一餐主食先少一點。"
    else:
        starch="主食看餓不餓再加，不需要為了達到每日數字刻意多吃。"

    oil=""
    if fat_high:
        oil="今天脂肪已接近建議量，下一餐優先清蒸、滷、烤、燙，炸物、肥肉、堅果大量攝取與濃醬先跳過。"

    small=""
    if 0 < cal < 300:
        small=f"今天約還有 {cal} kcal 的熱量參考空間，若真的餓再抓小份即可。"

    return "".join([lead,action,example_text,starch,oil,small,"剩餘數字只是參考空間，不代表一定要全部吃完。"]) 
...
15. 文字要像真人體態教練：短、直接、可執行；每個欄位最多1–2句。
16. good 必須講具體食物與具體優點；只有真的有明顯優點才寫。若沒有明顯優點，good 請輸出空字串，不要為了鼓勵硬找優點。
...
"estimate_note":"照片估算限制"
    ...
    # Keep single-meal labels separate from whole-day progress labels.
    data=_single_meal_statuses(data)

    # Prevent wording that makes a high-starch meal sound like the whole day is over target.
    overall=(data.get("overall") or "").strip()
    for old in ("澱粉量明顯偏多","碳水明顯偏多","澱粉明顯偏多"):
        overall=overall.replace(old,"主食比例較高")
    data["overall"]=overall
    good=(data.get("good") or "").strip()
    if good in ("這餐先不用特別稱讚某一項，重點看下一步怎麼調整。","沒有明顯優點。","無"):
        good=""
    data["good"]=good
    ...
    if timing=="after" and fullness_after not in (None,""):
        ...
        if full_i is not None and full_i>=7:
            if data["veg_fists"]==0:
                data["advice"]="下一次進食先補 1–2 拳蔬菜。"
            elif remaining_after["fat"] <= 8:
                data["advice"]="下一次進食改選低脂烹調，先避開炸物與濃醬。"
            elif progress["protein"]["status"]=="目前偏少":
                data["advice"]="下一次進食優先安排一份明顯的蛋白質主菜。"
            else:
                data["advice"]="這餐不用硬改，下一次進食照今天剩餘空間安排即可。"
...
def today_totals():
    today=datetime.now().date().isoformat(); c=db()
    rows=c.execute("SELECT calories,protein_g,carbs_g,fat_g,veg_fists,water_ml,meal_type,created_at FROM meals WHERE user_key=? AND substr(created_at,1,10)=? AND deleted_at IS NULL ORDER BY created_at",(user_key(),today)).fetchall()
...
def meal():
    ...
    if request.method=="POST":
        ...
        if today_usage()>=limit:
            ...
            return render_template("meal.html",analysis=None,error=f"今天已達 {limit} 次 AI 分析上限。",p=p,totals=totals,remaining=rem,today_progress=daily_nutrition_progress(p,totals),form_data={},usage=today_usage(),limit=limit,form_token=form_token)
        ...
        if not text and not raw:
            ...
            totals=today_totals(); rem=remaining(p,totals)
            return render_template("meal.html",analysis=None,error="請至少上傳一張餐點照片，或輸入餐點內容。",p=p,totals=totals,remaining=rem,today_progress=daily_nutrition_progress(p,totals),form_data=form_data,usage=today_usage(),limit=limit,form_token=secrets.token_urlsafe(24))
        def num(n):
    ...
    return render_template("meal.html",analysis=analysis,error=error,p=p,totals=totals,remaining=rem,today_progress=today_progress,form_data=form_data,usage=today_usage(),limit=int((membership() or {}).get("daily_limit") or 6),form_token=form_token)
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.7.8","database":"postgres" if USE_POSTGRES else "sqlite"}
