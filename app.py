...
def ai_analyze(text=None,image_bytes=None,p=None,timing="before",hunger_before=None,fullness_after=None,totals=None,meal_type=None):
    ...
    client=OpenAI(api_key=key, timeout=float(os.getenv("OPENAI_TIMEOUT_SECONDS","30")), max_retries=0)
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.5","database":"postgres" if USE_POSTGRES else "sqlite"}
