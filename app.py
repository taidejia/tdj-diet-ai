...
14. 嚴格分工，禁止重複：overall 只評論「本餐本身」；good 只寫「本餐做得好的地方」；advice 只寫「現在最值得做的一件事」。不要在這三欄重複今日累計、每日目標、剩餘克數，這些由系統下方的「今日整體營養進度」呈現。
15. good 最多1句，禁止使用「但是／但今天累計／距離每日目標／仍差」等轉折去講全天缺口。若本餐沒有明顯優點，就寫「這餐先不用特別稱讚某一項，重點看下一步怎麼調整。」
16. advice 最多1句，只給一個動作。例如「下一餐補1–2拳蔬菜」或「下一餐優先補蛋白質」。不要同時重講蛋白質、碳水、脂肪、蔬菜四項。
17. overall 最多2句，描述本餐份量、主要熱量來源與最明顯缺口即可；不要把全天營養進度再說一次。
18. next_meal 只輸出「由系統依今日剩餘額度安排」，不要自行產生數字或菜單；後端會用真實剩餘額度產生下一餐建議。
19. 避免同義反覆，例如「有吃到蛋白質／蛋白質來源完整／蛋白質有補到」只能選一種說法出現一次。
只輸出JSON：{{"calories":數字,"protein_g":數字,"carbs_g":數字,"fat_g":數字,"veg_fists":數字,"confidence":"高/中/低","protein_status":"足夠/可接受/偏少/偏多/無法判斷","carbs_status":"足夠/可接受/偏少/偏多/無法判斷","fat_status":"足夠/可接受/偏少/偏多/無法判斷","veg_status":"足夠/可接受/偏少/偏多/無法判斷","overall":"只評論本餐，最多2句，不寫全天累計","good":"只寫本餐優點，最多1句，不寫全天缺口","advice":"只寫現在最優先的一個動作，最多1句","remaining_after":"placeholder","next_meal":"由系統依今日剩餘額度安排","estimate_note":"估算限制"}}"""
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.6.6","database":"postgres" if USE_POSTGRES else "sqlite"}
