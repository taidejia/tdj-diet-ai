...
def daily_nutrition_progress(p, totals):
    ...
    # Carbs: this is allocation space, not a target that must be filled.
    ...
    if carbs_goal<=0:
        ...
        carbs_status="偏多"
        carbs_note=f"今天碳水約 {carbs_now:.0f}g，已高於參考配置；後續主食份量先減少。"
    elif carbs_now >= carbs_goal*0.65:
        carbs_status="目前可接受"
        carbs_note=f"今天碳水約 {carbs_now:.0f}g，目前在可接受配置內；主食依飢餓感安排，不需要刻意吃滿。"
    else:
        carbs_status="尚有配置空間"
        carbs_note=f"今天碳水約 {carbs_now:.0f}g，約還有 {carb_gap:.0f}g 的配置空間；依飢餓感安排即可，不用硬補滿。"
...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    ...
    if timing=="after" and fullness is not None and fullness>=7:
        parts.append("這餐吃完已經有飽足感，先不用再補。")
    else:
        parts.append("如果今天後續還會吃，")

    priorities=[]
    if protein_need:
        priorities.append("一份蛋白質主菜")
    if veg_need:
        priorities.append("1–2 拳蔬菜")

    if priorities:
        parts.append("下一餐先把今天比較缺的補回來："+"＋".join(priorities)+"。")
    else:
        parts.append("下一餐照正常份量、依飢餓感安排就可以。")

    if carbs_high:
        parts.append("主食先少一點。")
    else:
        parts.append("主食看餓不餓再加，不需要為了達到數字刻意多吃。")

    if fat_high:
        parts.append("今天油脂已經不少，下一餐盡量選清蒸、滷、烤、燙，炸物和濃醬先跳過。")

    if 0 < cal < 300:
        parts.append(f"今天大約只剩 {cal} kcal 的熱量參考空間，份量抓小份即可。")

    parts.append("剩餘數字是參考空間，不代表一定要全部吃完。")
    return "".join(parts)
...
16. good 必須講具體食物與具體優點，不要寫「不是只有單純澱粉」這種繞口比較。例：有牛肉/蛋/豆腐，就說「有安排一份蛋白質來源，這點有顧到。」
17. 不要用「補到」、「硬補」、「吃滿」來描述蛋白質目標；改用「優先安排」、「今天仍偏少」、「後續再補一些」等自然說法。
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.7.7","database":"postgres" if USE_POSTGRES else "sqlite"}
