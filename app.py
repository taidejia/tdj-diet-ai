...
import os, json, sqlite3, base64, requests, io, time, secrets
try:
    import psycopg
...
def normalize_meal_image(raw_bytes):
    """Convert common phone images to a compact standard JPEG for vision input."""
    if not raw_bytes:
        return None
    try:
        with Image.open(io.BytesIO(raw_bytes)) as im:
            ...
            if im.mode not in ("RGB","L"):
                # Flatten transparency against white before JPEG conversion.
                if "A" in im.getbands():
            ...
            data=out.getvalue()
            app.logger.warning("Meal photo normalized original=%d bytes jpeg=%d bytes size=%sx%s", len(raw_bytes), len(data), im.size[0], im.size[1])
            return data
    except UnidentifiedImageError:
        app.logger.exception("Unsupported/unreadable meal photo")
        raise ValueError("照片格式無法辨識，請改用 JPG、PNG 或重新選擇照片。")
    except Exception:
        app.logger.exception("Meal photo preprocessing failed")
        raise ValueError("照片處理失敗，請重新選擇照片後再試。")
...
def ai_analyze(text=None,image_bytes=None,p=None,timing="before",hunger_before=None,fullness_after=None,totals=None,meal_type=None):
    ...
    timeout_seconds=float(os.getenv("OPENAI_PHOTO_TIMEOUT_SECONDS","45") if image_bytes else os.getenv("OPENAI_TIMEOUT_SECONDS","30"))
    client=OpenAI(api_key=key, timeout=timeout_seconds, max_retries=0)
    started=time.monotonic()
    app.logger.warning("AI meal analysis started user=%s photo=%s", user_key(), bool(image_bytes))
...
6. 沒蔬菜：吃前可建議加1–2拳；吃完則下一餐補，不要求硬補。
...
只輸出JSON：{{"calories":數字,"protein_g":數字,"carbs_g":數字,"fat_g":數字,"veg_fists":數字,"confidence":"高/中/低","protein_status":"足夠/可接受/偏少/偏多/無法判斷","carbs_status":"足夠/可接受/偏少/偏多/無法判斷","fat_status":"足夠/可接受/偏少/偏多/無法判斷","veg_status":"足夠/可接受/偏少/偏多/無法判斷","overall":"一句整體判斷","good":"做得好的地方","advice":"現在最優先做的1件事；若不用硬改就明說","remaining_after":"placeholder","next_meal":"下一餐營養範圍＋2個台灣常見實際選擇","estimate_note":"估算限制"}}"""
    ...
    if image_bytes:
        image_bytes=normalize_meal_image(image_bytes)
    ...
    try:
        ...
        app.logger.warning("AI meal analysis finished in %.2fs user=%s", time.monotonic()-started, user_key())
    ...
    try:
        ...
        return None,"餐點分析暫時無法完成，請稍後再試。你剛填寫的內容已保留。"
...
def meal():
    ...
    if request.method=="POST":
        ...
        try:
            analysis,error=ai_analyze(text,raw,p,timing,num("hunger_before"),num("fullness_after"),pre_totals,meal_type)
        except ValueError as e:
            app.logger.exception("Meal photo validation/preprocessing failed")
            error=str(e)
        except Exception:
            app.logger.exception("AI meal analysis failed")
            error="餐點分析目前暫時無法使用，請稍後再試。你填寫的內容已保留，不需要重新輸入。"
        if analysis:
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.5.1","database":"postgres" if USE_POSTGRES else "sqlite"}
