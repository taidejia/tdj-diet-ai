...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    ...
    if timing=="after" and fullness is not None and fullness>=7:
        text="這餐吃完已經有飽足感，現在不用立刻補吃。"
    else:
        text="如果今天後續還會吃，"

    if cal <= 0:
        return text+"今天的熱量參考空間已用完；真的餓再以清淡、低熱量食物為主，不需要為了補數字硬吃。"

    priorities=[]
    if protein_need:
        priorities.append("一份明顯的蛋白質主菜")
    if veg_need:
        priorities.append("1–2 拳蔬菜")

    if priorities:
        text+="下一次進食優先安排"+"＋".join(priorities)+"。"
    else:
        text+="下一次進食依飢餓感安排一份均衡餐即可。"

    if carbs_high:
        text+="主食先少一點。"
    else:
        text+="主食看餓不餓再加，不需要為了吃滿每日碳水刻意補。"

    if fat_high:
        text+="今天脂肪已接近建議量，先避開炸物、肥肉、堅果與濃醬。"

    if cal < 350:
        text+=f"今天大約還有 {cal} kcal 的參考空間，份量抓小份即可。"

    text+="剩餘額度是參考空間，不代表一定要全部吃完。"
    return text
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.7.6","database":"postgres" if USE_POSTGRES else "sqlite"}
