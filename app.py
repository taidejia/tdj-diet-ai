...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    """Final client-facing coaching: priority first, concrete portions, no force-filling targets."""
    try:
    ...
    if timing=="after" and fullness is not None and fullness>=7:
        parts.append("這餐吃完已有飽足感，現在不用再加餐。")
    elif timing=="after":
        parts.append("這餐已經吃完，下一次真的餓了再吃。")

    if protein_need and veg_need:
        parts.append("下一餐優先安排蛋白質和蔬菜：肉或魚約 1–1.5 個掌心，搭配 1–2 拳蔬菜；也可以用豆腐＋蛋或無糖豆漿替代部分肉類。")
    elif protein_need:
        parts.append("下一餐優先把蛋白質顧好：肉或魚約 1–1.5 個掌心，也可以選豆腐＋蛋或無糖豆漿。")
    elif veg_need:
        parts.append("下一餐先補 1–2 拳蔬菜，例如燙青菜、炒青菜或蔬菜湯。")
    else:
        parts.append("下一餐照正常份量、依飢餓感安排即可。")

    if carbs_high:
        parts.append("今天主食已偏多，下一餐主食份量先縮小。")
    else:
        parts.append("主食依飢餓感決定，不需要為了達到每日數字刻意多吃。")

    if fat_high:
        parts.append("今天脂肪已接近建議量，下一餐優先清蒸、滷、烤、燙，炸物、肥肉與濃醬先跳過。")

    if 0 < cal < 250:
        parts.append(f"今天約還有 {cal} kcal 的熱量參考空間；若還會吃，份量抓小份即可。")

    parts.append("剩餘數字只是參考，不代表一定要全部吃完。")
    return "".join(parts)
    ...
    if timing=="after" and fullness_after not in (None,""):
        ...
        if full_i is not None and full_i>=7:
            if data["veg_fists"]==0:
                ...
                data["advice"]="下一次進食優先安排一份足量的蛋白質主菜。"
            else:
...
def meal():
    ...
    if request.method=="POST":
        ...
        if not analysis:
            ...
            form_token=secrets.token_urlsafe(24)
        if analysis:
            # History stores only this meal's snapshot. Whole-day progress and next-meal guidance change later,
            # so they must not be frozen into each historical meal record.
            history_parts=[f"整體：{analysis.get('overall','')}" ]
            if (analysis.get('good') or '').strip():
                history_parts.append(f"做得好的地方：{analysis.get('good','')}")
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-1.8.0","database":"postgres" if USE_POSTGRES else "sqlite"}
