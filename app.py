...
def result():
    ...
    issues,tasks=assess(p); red=safety(p)
    bmi=None
    if p.get("height") and p.get("weight"): bmi=round(p["weight"]/((p["height"]/100)**2),1)
    return render_template("result.html",p=p,issues=issues,tasks=tasks,red=red,bmi=bmi)
...
分析順序：1餐點辨識 2蛋白質/澱粉/蔬菜/脂肪大致份量 3這餐做得好的地方 4最值得調整的一件事
5依使用者今天目標給下一餐方向。若照片不清楚要說不確定。不要把單餐好壞等同減脂成敗。"""
