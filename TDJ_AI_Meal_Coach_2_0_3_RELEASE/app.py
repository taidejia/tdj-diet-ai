...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    ...
    if cal <= 300:
        ...
        guides=[]
        if protein_need or veg_need:
            guides.append("超商：茶葉蛋／雞胸／無糖豆漿擇一，搭沙拉或關東煮蔬菜")
            guides.append("滷味／鹽水雞：豆腐、蛋或少量雞肉＋2種蔬菜，少醬不淋油")
        else:
            guides.append("清湯、蔬菜或小份低油點心，依飢餓感選擇")
        if not carbs_high and c>=20:
            guides.append("若真的很餓，可再加小地瓜或少量飯")
        parts.append("外食可以這樣選："+"；".join(guides)+"。")
    ...
    if fat_high:
        guides.append(f"便當／自助餐：滷雞腿去皮、烤雞、白肉魚或瘦肉1份＋2樣青菜＋{rice}；炸排骨、肥肉、勾芡與濃醬先跳過。")
        guides.append("超商：雞胸肉或茶葉蛋＋沙拉／關東煮蔬菜；主食是否加，依今天剩餘碳水與飢餓感決定。")
        guides.append("滷味／鹽水雞：雞肉、豆腐或蛋擇2種＋2–3種蔬菜，少醬、不額外淋油。")
    elif protein_need and veg_need:
        guides.append(f"便當／自助餐：雞腿、雞胸、魚或瘦肉1份＋2樣青菜＋{rice}。")
        guides.append("超商：雞胸肉＋茶葉蛋＋沙拉；主食依今天剩餘碳水與飢餓感決定，飲料選無糖。")
        guides.append("滷味／鹽水雞：雞肉或瘦肉＋蛋／豆干＋2–3種蔬菜；有碳水空間再加一小份主食。")
    elif protein_need:
        guides.append(f"便當／自助餐：肉或魚1份＋至少1樣青菜＋{rice}。")
        guides.append("超商：雞胸肉＋茶葉蛋或無糖豆漿，再依今天剩餘碳水與飢餓感決定是否加主食。")
    elif veg_need:
        guides.append(f"便當／自助餐：原本主菜照吃，青菜直接選2樣＋{rice}。")
        guides.append("火鍋：先拿2拳以上蔬菜＋一份肉／魚／豆腐；丸餃少量，主食依今天剩餘碳水決定。")
        guides.append("麵店：若今天仍有碳水空間可吃小份麵，另外加燙青菜與蛋／豆干／瘦肉；若碳水已高就改湯菜＋蛋白質。")
    else:
        guides.append(f"便當／自助餐：一份肉或魚＋1–2樣青菜＋{rice}。")
        guides.append("超商：蛋白質＋沙拉／蔬菜，主食依今天剩餘碳水與飢餓感決定。")

    parts.append("外食可以這樣選："+" ".join(f"{i+1}) {g}" for i,g in enumerate(guides[:3])))
    parts.append("上面的份量是實用參考，不需要為了把剩餘數字吃滿而硬吃。")
    return "".join(parts)



def external_next_meal_guides(progress, remaining, timing="before", fullness_after=None):
    ...
    protein = "一份雞肉、魚、瘦肉、豆腐或蛋" if protein_need else "原本主菜正常份量"
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-2.0.3","database":"postgres" if USE_POSTGRES else "sqlite"}
