...
import os, json, sqlite3, base64, requests, time, secrets, gc, threading
...
APP_VERSION = "2.4.13"
HEALTH_VERSION = f"meal-coach-{APP_VERSION}"

app = Flask(__name__)
# Protect the 512 MB Render instance from unexpectedly huge multipart uploads.
# Five normal phone photos remain supported; oversized uploads fail early instead
# of forcing the worker to allocate hundreds of MB at once.
app.config["MAX_CONTENT_LENGTH"] = int(os.getenv("MAX_UPLOAD_MB", "40")) * 1024 * 1024
IMAGE_MAX_DIM = int(os.getenv("IMAGE_MAX_DIM", "2048"))
IMAGE_JPEG_QUALITY = int(os.getenv("IMAGE_JPEG_QUALITY", "82"))
IMAGE_MAX_SOURCE_MB = int(os.getenv("IMAGE_MAX_SOURCE_MB", "15"))
Image.MAX_IMAGE_PIXELS = 50_000_000

# 2.4.11: process-local FIFO admission queue for expensive AI meal analysis.
# It caps simultaneous image/text AI analyses so a burst of users cannot all
# allocate image/Base64/OpenAI payload memory at once on a 512 MB instance.
AI_MAX_CONCURRENT = max(1, int(os.getenv("AI_MAX_CONCURRENT", "2")))
AI_QUEUE_TIMEOUT = max(30, int(os.getenv("AI_QUEUE_TIMEOUT", "180")))
_ai_queue_cond = threading.Condition()
_ai_waiting = []
_ai_active = 0

def ai_queue_snapshot():
    with _ai_queue_cond:
        return {"active": _ai_active, "waiting": len(_ai_waiting), "limit": AI_MAX_CONCURRENT}

def acquire_ai_slot(user_label=""):
    """FIFO admission control. Returns (ticket, waited_seconds, position_at_join)."""
    global _ai_active
    ticket = secrets.token_urlsafe(10)
    started = time.monotonic()
    with _ai_queue_cond:
        _ai_waiting.append(ticket)
        join_pos = len(_ai_waiting)
        app.logger.info("AI_QUEUE_JOIN user=%s ticket=%s position=%s active=%s limit=%s",
                        user_label, ticket, join_pos, _ai_active, AI_MAX_CONCURRENT)
        while True:
            is_head = bool(_ai_waiting) and _ai_waiting[0] == ticket
            if is_head and _ai_active < AI_MAX_CONCURRENT:
                _ai_waiting.pop(0)
                _ai_active += 1
                waited = time.monotonic() - started
                app.logger.info("AI_QUEUE_START user=%s ticket=%s waited=%.3fs active=%s waiting=%s",
                                user_label, ticket, waited, _ai_active, len(_ai_waiting))
                return ticket, waited, join_pos
            remaining = AI_QUEUE_TIMEOUT - (time.monotonic() - started)
            if remaining <= 0:
                try: _ai_waiting.remove(ticket)
                except ValueError: pass
                _ai_queue_cond.notify_all()
                app.logger.warning("AI_QUEUE_TIMEOUT user=%s ticket=%s active=%s waiting=%s",
                                   user_label, ticket, _ai_active, len(_ai_waiting))
                return None, time.monotonic() - started, join_pos
            _ai_queue_cond.wait(timeout=min(1.0, remaining))

def release_ai_slot(ticket, user_label=""):
    global _ai_active
    if not ticket:
        return
    with _ai_queue_cond:
        _ai_active = max(0, _ai_active - 1)
        app.logger.info("AI_QUEUE_DONE user=%s ticket=%s active=%s waiting=%s",
                        user_label, ticket, _ai_active, len(_ai_waiting))
        _ai_queue_cond.notify_all()

def _rss_mb():
    """Best-effort current RSS on Linux/Render; returns None elsewhere."""
    try:
...
def log_memory(stage, **extra):
    rss=_rss_mb()
    app.logger.info("MEMORY stage=%s rss_mb=%s extra=%s", stage, rss, extra or {})
...
def member_admin_state(row):
    """Admin-facing effective state: unopened, active, expired, or inactive.

    A LINE login creates a membership row immediately with status=inactive.
    If that row has never had a plan/start/expiry, it is a newly logged-in
    member waiting for TDJ to open access, not a manually disabled student.
    """
    try:
        get=lambda k: (row.get(k) if hasattr(row,"get") else row[k])
        status=(get("status") or "inactive")
        expires=get("expires_at")
        starts=get("starts_at")
        program_start=get("program_starts_at")
    except Exception:
        return "inactive"

    if status != "active":
        # LINE-authenticated but never opened by TDJ.  Manual deactivation keeps
        # the historical plan timestamps, so it remains distinguishable.
        if not starts and not program_start and not expires:
            return "unopened"
        return "inactive"
    if not expires:
        return "inactive"
    try:
        return "active" if datetime.fromisoformat(str(expires)) >= utc_now_naive() else "expired"
    except Exception:
        return "inactive"

app.jinja_env.globals["member_admin_state"] = member_admin_state
...
def init_db():
    ...
    if USE_POSTGRES:
        ...
        CREATE TABLE IF NOT EXISTS checkins(
          id BIGSERIAL PRIMARY KEY,
          user_key TEXT, weight DOUBLE PRECISION, body_fat DOUBLE PRECISION, waist DOUBLE PRECISION,
          hunger INTEGER, sleep_hours DOUBLE PRECISION, sleep_quality TEXT, bowel_status TEXT,
          menstrual_status TEXT, fatigue_level INTEGER, activity_note TEXT, water INTEGER, note TEXT, created_at TEXT
        );
        ...
        CREATE TABLE IF NOT EXISTS fasting_settings(
          user_key TEXT PRIMARY KEY, enabled INTEGER DEFAULT 0, mode TEXT DEFAULT '16:8',
          fasting_hours DOUBLE PRECISION DEFAULT 16, eating_hours DOUBLE PRECISION DEFAULT 8,
          default_start_time TEXT DEFAULT '22:00', updated_at TEXT
        );
        CREATE TABLE IF NOT EXISTS fasting_sessions(
          id BIGSERIAL PRIMARY KEY, user_key TEXT NOT NULL, started_at TEXT NOT NULL,
          target_end_at TEXT NOT NULL, fasting_hours DOUBLE PRECISION NOT NULL, created_at TEXT
        );
        CREATE TABLE IF NOT EXISTS adaptive_reviews(
          id BIGSERIAL PRIMARY KEY, user_key TEXT NOT NULL, checkpoint INTEGER NOT NULL,
          review_text TEXT NOT NULL, status TEXT, created_at TEXT
        );
        ...
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS waist DOUBLE PRECISION;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS hunger INTEGER;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS sleep_hours DOUBLE PRECISION;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS sleep_quality TEXT;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS bowel_status TEXT;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS menstrual_status TEXT;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS fatigue_level INTEGER;
        ALTER TABLE checkins ADD COLUMN IF NOT EXISTS activity_note TEXT;
        ...
        CREATE TABLE IF NOT EXISTS analysis_jobs(
          ...
          result_meal_id BIGINT,
          error TEXT,
          created_at TEXT NOT NULL,
          started_at TEXT,
          finished_at TEXT
        );
        CREATE TABLE IF NOT EXISTS analysis_job_photos(
          id BIGSERIAL PRIMARY KEY,
          job_id BIGINT NOT NULL,
          data_url TEXT NOT NULL,
          created_at TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS idx_analysis_jobs_status_id ON analysis_jobs(status,id);
        CREATE INDEX IF NOT EXISTS idx_analysis_jobs_user_created ON analysis_jobs(user_key,created_at);
        CREATE INDEX IF NOT EXISTS idx_analysis_job_photos_job ON analysis_job_photos(job_id);
    ...
    CREATE TABLE IF NOT EXISTS checkins(
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_key TEXT, weight REAL, body_fat REAL, waist REAL,
      hunger INTEGER, sleep_hours REAL, sleep_quality TEXT, bowel_status TEXT, menstrual_status TEXT,
      fatigue_level INTEGER, activity_note TEXT, water INTEGER, note TEXT, created_at TEXT
    );
    """)
    ...
    CREATE TABLE IF NOT EXISTS fasting_settings(
      user_key TEXT PRIMARY KEY, enabled INTEGER DEFAULT 0, mode TEXT DEFAULT '16:8',
      fasting_hours REAL DEFAULT 16, eating_hours REAL DEFAULT 8, default_start_time TEXT DEFAULT '22:00', updated_at TEXT
    );
    CREATE TABLE IF NOT EXISTS fasting_sessions(
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_key TEXT NOT NULL, started_at TEXT NOT NULL,
      target_end_at TEXT NOT NULL, fasting_hours REAL NOT NULL, created_at TEXT
    );
    CREATE TABLE IF NOT EXISTS adaptive_reviews(
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_key TEXT NOT NULL, checkpoint INTEGER NOT NULL,
      review_text TEXT NOT NULL, status TEXT, created_at TEXT
    );
    """)
    checkin_cols={r[1] for r in c.execute("PRAGMA table_info(checkins)").fetchall()}
    for col,typ in [("waist","REAL"),("hunger","INTEGER"),("sleep_hours","REAL"),("sleep_quality","TEXT"),("bowel_status","TEXT"),("menstrual_status","TEXT"),("fatigue_level","INTEGER"),("activity_note","TEXT")]:
        if col not in checkin_cols: c.execute(f"ALTER TABLE checkins ADD COLUMN {col} {typ}")
...
def day_totals(day=None):
    """Nutrition/water totals for one Taiwan calendar day (defaults to today)."""
    start_utc,end_utc=taipei_day_utc_bounds(day); c=db()
    ...
    return {
        ...
        "meal_water":int(sum((r["water_ml"] or 0) for r in rows)),
        "extra_water":int(sum((r["amount_ml"] or 0) for r in water_rows)),
        "meal_count":len(rows),
        "last_meal_type":last_meal_type
    }

def day_usage(day=None):
    start_utc,end_utc=taipei_day_utc_bounds(day); c=db()
...
def meal_recorded_at(day):
    """Backdate a meal to the selected Taiwan day while preserving the current local clock time."""
    now=taipei_now()
    local=datetime(day.year,day.month,day.day,now.hour,now.minute,now.second,tzinfo=TAIPEI_TZ)
...
def fasting_setting():
    c=db(); r=c.execute("SELECT * FROM fasting_settings WHERE user_key=?",(user_key(),)).fetchone(); c.close()
    return dict(r) if r else {"enabled":0,"mode":"16:8","fasting_hours":16,"eating_hours":8,"default_start_time":"22:00"}

def fasting_state():
    st=fasting_setting()
    if not int(st.get("enabled") or 0): return {"enabled":False}
    c=db(); r=c.execute("SELECT * FROM fasting_sessions WHERE user_key=? ORDER BY started_at DESC LIMIT 1",(user_key(),)).fetchone(); c.close()
    if not r:return {"enabled":True,"active":False,"setting":st}
    r=dict(r)
    try:
        start=datetime.fromisoformat(r["started_at"]).replace(tzinfo=timezone.utc).astimezone(TAIPEI_TZ)
        end=datetime.fromisoformat(r["target_end_at"]).replace(tzinfo=timezone.utc).astimezone(TAIPEI_TZ)
        now=taipei_now()
        if now < end:
            sec=max(0,int((end-now).total_seconds())); h=sec//3600; m=(sec%3600)//60
            return {"enabled":True,"active":True,"phase":"fasting","start":start,"end":end,"remaining":f"{h}小時{m}分", "setting":st}
        eat_end=end+timedelta(hours=float(st.get("eating_hours") or 8))
        if now < eat_end:
            sec=max(0,int((eat_end-now).total_seconds())); h=sec//3600; m=(sec%3600)//60
            return {"enabled":True,"active":True,"phase":"eating","start":start,"end":end,"eat_end":eat_end,"remaining":f"{h}小時{m}分", "setting":st}
        return {"enabled":True,"active":False,"last_end":end,"setting":st}
    except Exception:
        return {"enabled":True,"active":False,"setting":st}
...
def fasting():
    ...
    if g:return g
    p=get_profile() or {}
    unsafe=(p.get("pregnant")=="yes" or p.get("breastfeeding")=="yes" or p.get("risk_eating")=="yes")
    if request.method=="POST":
        action=request.form.get("action")
        if action=="save":
            enabled=1 if request.form.get("enabled")=="1" else 0
            if enabled and unsafe:
                flash("目前個人設定含不適合自行啟用間歇性斷食的情況，請先詢問醫療專業人員。")
                return redirect(url_for("fasting"))
            mode=request.form.get("mode","16:8")
            presets={"14:10":(14,10),"16:8":(16,8),"18:6":(18,6)}
            if mode in presets: fh,eh=presets[mode]
            else:
                try: fh=float(request.form.get("fasting_hours") or 16); eh=24-fh
                except: fh,eh=16,8
                fh=max(12,min(20,fh)); eh=24-fh; mode="自訂"
            t=request.form.get("default_start_time") or "22:00"
            c=db(); c.execute("DELETE FROM fasting_settings WHERE user_key=?",(user_key(),)); c.execute(
                "INSERT INTO fasting_settings(user_key,enabled,mode,fasting_hours,eating_hours,default_start_time,updated_at) VALUES(?,?,?,?,?,?,?)",
                (user_key(),enabled,mode,fh,eh,t,db_now_iso())); c.commit(); c.close()
            flash("168／進食時間設定已更新。")
        elif action=="start":
            st=fasting_setting()
            ...
            if unsafe:
                flash("目前個人設定含不適合自行啟用間歇性斷食的情況，請先詢問醫療專業人員。")
                return redirect(url_for("fasting"))
            t=request.form.get("start_time") or taipei_now().strftime("%H:%M")
            hh,mm=[int(x) for x in t.split(":")]
            local=taipei_now().replace(hour=hh,minute=mm,second=0,microsecond=0)
            # If a manual time is far ahead, interpret it as yesterday's start (e.g. entering 22:00 after midnight).
            if local > taipei_now()+timedelta(minutes=5): local-=timedelta(days=1)
            start_utc=local.astimezone(timezone.utc).replace(tzinfo=None)
            end_utc=start_utc+timedelta(hours=float(st.get("fasting_hours") or 16))
            c=db(); c.execute("INSERT INTO fasting_sessions(user_key,started_at,target_end_at,fasting_hours,created_at) VALUES(?,?,?,?,?)",
                (user_key(),start_utc.isoformat(timespec="seconds"),end_utc.isoformat(timespec="seconds"),float(st.get("fasting_hours") or 16),db_now_iso())); c.commit(); c.close()
            flash(f"已從 {local.strftime('%H:%M')} 開始計算，預計 {end_utc.replace(tzinfo=timezone.utc).astimezone(TAIPEI_TZ).strftime('%m/%d %H:%M')} 可開始進食。")
        return redirect(url_for("fasting"))
    return render_template("fasting.html",setting=fasting_setting(),state=fasting_state(),unsafe=unsafe)

@app.route("/")
def home():
    p=get_profile()
    active=member_active()
    # Active first-time members must finish the one-time setup before using the coach.
    if active and current_line_user() and not p:
    ...
    return render_template("home.html",profile=p,line_user=current_line_user(),member=membership(),active=active,totals=totals,progress=progress,remaining=rem,target_meta=calculate_targets(p) if p else None,fasting_state=fasting_state() if active and p else {"enabled":False},body_completion=checkin_completion(p) if active and p else None)

def save_profile_form(f):
...
def settings():
    ...
    if g:return g
    p=get_profile()
    if not p:return redirect(url_for("onboarding"))
    if request.method=="POST":
        save_profile_form(request.form)
        flash("我的設定已更新，系統已重新計算目前的每日參考目標。")
        return redirect(url_for("result"))
    return render_template("settings.html",profile=p)

@app.route("/assessment",methods=["GET","POST"])
def assessment():
    # Backward-compatible route for old bookmarks/links.
    return redirect(url_for("settings" if get_profile() else "onboarding"))
...
def _single_meal_statuses(data):
    ...
    data["fat_status"]="脂肪較高" if fat>=25 else ("脂肪中等" if fat>=10 else "脂肪較低")
...
def _meal_overall_copy(data):
    ...
    if f>=25:
        fat_type=(data.get("fat_source_type") or "").strip()
        fat_source=(data.get("fat_source_note") or "").strip()
        if fat_type=="烹調油脂為主":
            points.append("烹調油脂偏高")
        elif fat_type=="食物本身脂肪為主" and fat_source:
            points.append(f"脂肪較高，主要來自{fat_source}")
        else:
            points.append("脂肪較高")
    elif f>=10: points.append("脂肪中等")
    else: points.append("脂肪較低")
...
def _meal_advice_copy(data):
    ...
    f=float(data.get("fat_g",0) or 0); v=float(data.get("veg_fists",0) or 0)
    if v<0.5 and f>=25:
        return "這餐最優先需要調整的是蔬菜不足。下一次進食先安排 1–2 拳蔬菜；脂肪來源則依本餐食物與烹調方式另外判斷，不直接等同於吃得太油。"
    if v<0.5:
    ...
    if f>=25:
        ...
        fat_source=(data.get("fat_source_note") or "").strip()
        if fat_type=="烹調油脂為主":
            return "這餐較需要注意的是烹調油脂。下一餐可優先選清蒸、滷、烤或燙的料理。"
        if fat_type=="食物本身脂肪為主":
            source_text=f"，主要來自{fat_source}" if fat_source else ""
            return f"這餐脂肪量較高{source_text}，但不等於料理過油；下一餐正常選較低脂的蛋白質與蔬菜即可。"
        return "這餐脂肪量較高，但目前無法確定主要來自食物本身還是烹調用油；下一餐可選較低脂、少濃醬的料理。"
    if c>=70:
...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    ...
    fat_high=(fat_status in ("已接近今日建議量","偏多"))
...
def _optimize_uploaded_image(file_storage):
    """Return a review-quality JPEG while keeping peak RAM bounded.

    The previous implementation read every original phone photo into a list,
    then produced additional Base64 copies for OpenAI and PostgreSQL. A handful
    of 12–48 MP photos could therefore exceed a 512 MB Render worker even with
    only one user. We decode one image at a time, normalize orientation, shrink
    the long edge, and keep only the compressed result in memory.
    """
    if not file_storage or not getattr(file_storage, "filename", None):
        return None
    max_source = IMAGE_MAX_SOURCE_MB * 1024 * 1024
    try:
        # FileStorage normally points to a SpooledTemporaryFile, so Pillow can
        # read from the stream without first copying the entire original file.
        ...
        # JPEG draft asks the decoder for a smaller working image when possible,
        # reducing peak decompression memory for very large phone photos.
        if (im.format or "").upper()=="JPEG":
            try: im.draft("RGB", (IMAGE_MAX_DIM, IMAGE_MAX_DIM))
            except Exception: pass
        im=ImageOps.exif_transpose(im)
        if im.mode not in ("RGB", "L"):
            # Flatten transparency onto white rather than carrying a large RGBA
            # buffer into the API/storage path.
            if "A" in im.getbands():
            ...
            im=im.convert("RGB")
        im.thumbnail((IMAGE_MAX_DIM, IMAGE_MAX_DIM), Image.Resampling.LANCZOS)
        out=BytesIO()
        im.save(out, format="JPEG", quality=IMAGE_JPEG_QUALITY, optimize=False, progressive=False)
        result=out.getvalue()
        try: im.close()
        except Exception: pass
        out.close()
        ...
        # Preserve compatibility for a browser-supported image format Pillow
        # cannot decode. Only allow a bounded raw fallback.
        try:
            stream=file_storage.stream
            stream.seek(0,2); size=stream.tell(); stream.seek(0)
            if size > max_source:
                raise ValueError(f"單張照片超過 {IMAGE_MAX_SOURCE_MB} MB，請先縮小照片後再上傳。")
            return stream.read()
        except ValueError:
            raise
        except Exception as e:
            raise ValueError("照片格式無法讀取，請改用 JPG、PNG 或 WebP。") from e
...
def _analyze_one_uploaded_image(client, raw, image_index):
    """Analyze one uploaded image independently before any multi-image merge.

    This prevents a clear nutrition label / side dish / drink from being silently
    dropped when several photos are submitted together.  The result is only a
    compact evidence summary; the final meal analysis still decides duplication
    and totals across the whole set.
    """
    b64=base64.b64encode(raw).decode()
    prompt=f"""你正在做多圖餐點分析的第 {image_index} 張「獨立預讀」。
這一步只能看這一張照片，不可以假設其他照片內容。請把這張照片中看得到的資訊完整列出，尤其不要忽略包裝食品或營養標示。

規則：
1. 先判斷 image_type：food（食物本體）/ nutrition_label（營養標示）/ package（包裝或成分表）/ mixed（同時有食物與標示）/ unclear。
2. 若有營養標示，不限中文、英文、日文、韓文或其他語言，都要嘗試讀出：每份量、每包幾份、熱量、蛋白質、碳水、脂肪、膳食纖維；讀不到的欄位填 null，不要亂猜。
3. 若是一般食物，列出可見食物、飲料、湯、小菜與大概份量，不要因為看起來像另一張照片可能重複就自行忽略。
4. 如果只有營養標示、看不到食物，仍視為有效資訊。
5. 請提供一個短的 identity_hint，用來讓下一步判斷它是否和其他照片是同一產品／同一盤的不同角度，例如品牌/品名/包裝顏色/餐點特徵；無法判斷則留空。

只輸出 JSON：
{{
  "image_index": {image_index},
  "image_type": "food/nutrition_label/package/mixed/unclear",
  "identity_hint": "",
  "visible_items": [""],
  "nutrition_label": {{
    "basis": "每份/每包/每100g/其他/",
    "serving_size": "",
  ...
  "notes": ""
    ...
    try:
        ...
        txt=_strip_json_fence(getattr(resp,"output_text","") or "")
        data=json.loads(txt)
        if not isinstance(data,dict):
            raise ValueError("single-image result is not object")
        data["image_index"]=image_index
        return data
    except Exception:
        app.logger.exception("SINGLE_IMAGE_PREVIEW_ERROR image_index=%s",image_index)
        return {
            "image_index":image_index,
            "image_type":"unclear",
            "identity_hint":"",
            "visible_items":[],
            "nutrition_label":{},
            "notes":"此張獨立預讀失敗；最終合併分析仍需直接檢查原圖，不得因此忽略這張照片。"
        }

def _preanalyze_uploaded_images(client, images):
    """Return one evidence record per image, preserving every uploaded photo.

    Independent image reads run concurrently so a 3–5 image meal does not
    become 3–5 sequential model waits before the final merge.
    """
    ...
    with ThreadPoolExecutor(max_workers=min(2,len(selected))) as pool:
        future_map={
            pool.submit(_analyze_one_uploaded_image,client,raw,idx): idx-1
        ...
        for fut in as_completed(future_map):
            ...
            try:
                ...
                app.logger.exception("SINGLE_IMAGE_PREVIEW_FUTURE_ERROR image_index=%s",pos+1)
                results[pos]={
                    "image_index":pos+1,"image_type":"unclear","identity_hint":"",
                    "visible_items":[],"nutrition_label":{},
                    "notes":"此張獨立預讀失敗；最終合併分析仍需直接檢查原圖，不得因此忽略這張照片。"
                }
    return results
...
def ai_analyze(text=None,image_bytes_list=None,p=None,timing="before",hunger_before=None,fullness_after=None,totals=None,meal_type=None):
    ...
    log_memory("ai_analyze_start", images=len(image_bytes_list or []), text_chars=len(text or ""))
    ...
    # Multi-image submissions are pre-read one-by-one first.  This prevents the
    # final merge from silently dropping a clear label, drink, side dish or
    # differently-language package simply because other images dominate attention.
    preview_started=time.perf_counter()
    image_evidence=_preanalyze_uploaded_images(client,images)
    if image_evidence:
        app.logger.info("MULTI_IMAGE_PREVIEW images=%s elapsed=%.3fs",len(image_evidence),time.perf_counter()-preview_started)
    ...
    image_evidence_text=(json.dumps(image_evidence,ensure_ascii=False) if image_evidence else "單張照片，無需多圖預讀")

    prompt=f"""你是 TDJ AI 餐盤教練。所有上傳照片都屬於同一餐。

【多張照片的強制處理順序】
A. 每一張照片都必須先被獨立看過，不准因為其他照片較清楚、語言不同、看起來像包裝背面，就直接忽略。
B. 下方提供的是系統對每張照片做的「獨立預讀證據」。最終分析必須逐張核對原圖與證據，確認 image 1、image 2... 每張都有被 accounted for。
C. 逐張辨識完成後，才做跨圖去重：只有能合理確認是「同一產品／同一盤／同一份食物的不同角度」時才合併；若不能確認相同，就當作不同品項納入同一餐。
D. 同一產品若一張是食物／正面、一張是營養標示，營養標示用來校正該產品，不可把同一份食物算兩次。
E. 不同產品各自有營養標示時，必須各自納入，不可因為都是包裝標示就只採其中一張。
F. 若獨立預讀某張失敗，仍必須直接看原圖完成最終判斷，不得把該張略過。
...
7A. 在進行一般餐點目測估算前，必須先逐張檢查是否有「包裝食品營養標示／Nutrition Facts」。不限語言，中文、英文、日文、韓文或其他語言都要嘗試辨識；不要因為不是中文就放棄。
7B. 若照片中存在可讀取的營養標示，先辨識欄位語意再擷取數字，例如韓文的 열량=熱量、단백질=蛋白質、지방=脂肪、탄수화물=碳水化合物；日文／英文同理。不要要求使用者自行翻譯。
7C. 有清楚營養標示時，標示數值的優先級高於食物外觀估算。先判斷標示基準是「每份／每包／每100g」，再依照片或文字可確認的實際食用份數換算；若實際食用份數無法確認，預設記錄 1 份並在 estimate_note 清楚說明。
7D. 如果照片只有包裝背面／營養標示，沒有看到食物本體，仍然是有效餐點資料，不得因「看不到食物」而回傳無法分析。
7E. 若營養標示的熱量、蛋白質、碳水、脂肪皆清楚可讀，nutrition_label_used=true，calories_low／estimate／high 應以標示換算值為中心，原則上不要再套一般照片 ±20% 的不確定範圍；confidence 可為高。只有部分欄位模糊時，才對無法讀到的部分保守估算並說明。
7F. 多張照片可能同時包含「食物本體＋營養標示＋成分表」。同一產品的標示只能套用一次，不得把包裝數值與食物目測熱量重複相加。
8. 多張照片要先去重，不能同一盤算兩次。
...
18. 單餐 status 只描述這一餐的結構，不代表全天是否達標：protein_status=本餐有沒有明顯蛋白質來源；carbs_status=本餐主食比例；fat_status=本餐脂肪量；veg_status=本餐蔬菜量。
18A. 「脂肪量較高」與「烹調過油」是兩件不同的事，禁止只因 fat_g 較高就寫成「太油／油脂偏高／吃得油」。
18B. 鮭魚、鯖魚、蛋、堅果、酪梨等食物本身含脂肪時，若主要脂肪來自食材本身，fat_source_type=食物本身脂肪為主；不要把它描述成烹調油脂問題。
18C. 炸物、麻油料理、三杯、明顯油煎、大量炒油、濃厚油醬等，只有在照片、菜名或文字有足夠依據時，才可判定 fat_source_type=烹調油脂為主。
18D. 若食物本身脂肪與烹調油都有明顯貢獻，fat_source_type=混合；無法判斷則填無法確定。fat_source_note 只寫主要來源，例如「鯖魚本身」或「油炸與醬汁」，不要自行臆測看不到的用油量。
...
"fat_status":"脂肪較高/脂肪中等/脂肪較低",
"fat_source_type":"食物本身脂肪為主/烹調油脂為主/混合/無法確定",
"fat_source_note":"簡短寫主要脂肪來源；無法確定則留空",
...
"nutrition_label_used":true或false,
"images_accounted_for":[1,2],
"image_merge_note":"多圖時簡短說明哪些照片被視為同一品項、哪些是不同品項；單圖留空",
"label_basis":"若有營養標示則填每份/每包/每100g/其他；沒有則留空",
"label_servings_used":數字,
"estimate_note":"照片估算限制；若使用營養標示，說明讀取的基準與換算方式"
}}"""
    ...
    # Long banquet / hot-pot / buffet text can contain many separate foods.
    # Give the model enough room for a complete JSON object instead of using a
    # fixed 1200-token ceiling that can cut off the response mid-object.
    text_lines=[ln.strip() for ln in (text or "").splitlines() if ln.strip()]
    item_hint=max(len(text_lines), len(image_evidence), 1)
    if item_hint >= 16:
        initial_tokens=3600
    elif item_hint >= 10:
        initial_tokens=3000
    ...
    def _request_analysis(max_tokens):
        ...
        app.logger.info(
            "OPENAI_REQUEST elapsed=%.3fs max_output_tokens=%s item_hint=%s status=%s",
            time.perf_counter()-call_started,max_tokens,item_hint,getattr(response,"status",None)
        )
        return response
    ...
    try:
        resp=_request_analysis(initial_tokens)
    except Exception:
        app.logger.exception("AI_ANALYZE_ERROR")
        try: client.close()
        except Exception: pass
        gc.collect()
        log_memory("ai_analyze_api_error")
        return None,"餐點分析暫時無法完成，請稍後再試。"

    data,txt=_extract_json(resp)
    status=getattr(resp,"status",None)
    incomplete_details=getattr(resp,"incomplete_details",None)
    truncated=(status=="incomplete")
    if incomplete_details:
        reason=getattr(incomplete_details,"reason",None)
        truncated=truncated or reason=="max_output_tokens"

    # One automatic retry for truncated / malformed JSON.  The user should not
    # have to press Analyse again just because a complex meal needed more room.
    if data is None:
        retry_tokens=max(4200, initial_tokens+1200)
        app.logger.warning(
            "AI_OUTPUT_RETRY status=%s truncated=%s initial_tokens=%s retry_tokens=%s raw_prefix=%r",
            status,truncated,initial_tokens,retry_tokens,txt[:500]
        )
        try:
            resp=_request_analysis(retry_tokens)
            data,txt=_extract_json(resp)
        except Exception:
            app.logger.exception("AI_ANALYZE_RETRY_ERROR")
            data=None

    if data is None:
        app.logger.error("AI_JSON_PARSE_ERROR raw=%r",txt[:1200])
        try: client.close()
        except Exception: pass
        try: del resp, content, image_evidence
        except Exception: pass
        gc.collect()
        log_memory("ai_analyze_json_error")
        return None,"餐點分析暫時無法完成，請稍後再試。"
    ...
    data["nutrition_label_used"]=bool(data.get("nutrition_label_used",False))
    accounted=data.get("images_accounted_for") or []
    try:
        accounted=sorted({int(x) for x in accounted if 1 <= int(x) <= min(len(images),5)})
    except Exception:
        accounted=[]
    data["images_accounted_for"]=accounted
    data["image_merge_note"]=(data.get("image_merge_note") or "").strip()
    expected=list(range(1,min(len(images),5)+1))
    if len(images)>1 and accounted != expected:
        app.logger.warning("MULTI_IMAGE_INCOMPLETE accounted=%s expected=%s",accounted,expected)
        data["estimate_note"]=(data.get("estimate_note","")+" 多圖分析已逐張預讀；部分照片在最終合併回傳未列出索引，請以拆解項目與原始照片人工複核。").strip()
    data["label_basis"]=(data.get("label_basis") or "").strip()
    try:data["label_servings_used"]=float(data.get("label_servings_used",1) or 1)
    except:data["label_servings_used"]=1.0
    ...
    if macro_kcal>0 and est>0 and not data.get("nutrition_label_used"):
    ...
    if data.get("nutrition_label_used"):
        # Package nutrition facts are measured label data, not a visual portion estimate.
        # Preserve label precision and do not artificially widen a photo-only range.
        data["calories_low"]=round(low,1)
        data["calories_estimate"]=round(est,1)
        data["calories_high"]=round(high,1)
    else:
    ...
    if images and not text and data["calories_estimate"]>0 and not data.get("nutrition_label_used"):
    ...
    for comp in comps:
        try:
            ...
            if data.get("nutrition_label_used"):
                comp["calories_low"]=round(cl,1)
                comp["calories_high"]=round(ch,1)
            else:
    ...
    if valid_components and not data.get("nutrition_label_used"):
    ...
    # Explicitly close the per-request OpenAI/httpx connection pool and drop
    # image/Base64 payload references before returning to Flask. This prevents
    # request-sized buffers from lingering until a later GC cycle.
    try: client.close()
    except Exception: pass
    try: del resp, content, image_evidence, image_evidence_text
    except Exception: pass
    gc.collect()
    log_memory("ai_analyze_done", images=len(images))
    return data,None
...
def _job_num(v):
    try: return int(v or 0) or None
    except: return None

def enqueue_analysis_job(user_key_value, submitted_token, payload, raws):
    """Durably enqueue one meal analysis in PostgreSQL.

    The web request only compresses/uploads the photos and writes the job. The
    expensive OpenAI work is performed by worker.py, so the browser can leave.
    """
    if not USE_POSTGRES:
        raise RuntimeError("背景分析需要 PostgreSQL DATABASE_URL")
    c=db()
    cur=c.execute("""INSERT INTO analysis_jobs(user_key,submission_token,status,payload,created_at)
                     VALUES(?,?,?,?,?) RETURNING id""",
                  (user_key_value,submitted_token,"queued",json.dumps(payload,ensure_ascii=False),db_now_iso()))
    ...
    for b in raws[:5]:
        ...
        c.execute("INSERT INTO analysis_job_photos(job_id,data_url,created_at) VALUES(?,?,?)",(job_id,data_url,db_now_iso()))
        del data_url
    c.commit(); c.close()
    return job_id

def pending_analysis_jobs(user_key_value, days=7):
    if not USE_POSTGRES: return []
    since=(utc_now_naive()-timedelta(days=days)).isoformat(timespec="seconds")
    c=db(); rows=c.execute("""SELECT id,status,payload,error,created_at,finished_at,result_meal_id
                              FROM analysis_jobs WHERE user_key=? AND created_at>=?
                              ORDER BY id DESC LIMIT 30""",(user_key_value,since)).fetchall(); c.close()
    out=[]
    for r in rows:
        d=dict(r)
        try: d["payload_obj"]=json.loads(d.get("payload") or "{}")
        except: d["payload_obj"]={}
        out.append(d)
    return out
...
def meal():
    ...
    if request.method=="POST":
        record_choice=(request.form.get("record_date") or "today").strip()
        today_day=taipei_now().date(); yesterday_day=today_day-timedelta(days=1)
        record_day=yesterday_day if record_choice=="yesterday" else today_day
        is_backfill=(record_day==yesterday_day)
        ...
        if day_usage(record_day)>=limit:
            totals=today_totals(); rem=remaining(p,totals)
            day_label="昨天" if is_backfill else "今天"
            return render_template("meal.html",analysis=None,error=f"{day_label}已達 {limit} 筆餐點紀錄上限。",p=p,totals=totals,remaining=rem,today_progress=daily_nutrition_progress(p,totals),target_meta=calculate_targets(p),form_data={"record_date":record_choice},usage=today_usage(),limit=limit,form_token=form_token)
        ...
        try:
            for img in imgs[:5]:
                if img and img.filename:
                    b=_optimize_uploaded_image(img)
                    if b: raws.append(b)
        except ValueError as e:
            c=db(); c.execute("DELETE FROM submission_tokens WHERE token=? AND user_key=?",(submitted_token,user_key())); c.commit(); c.close()
            totals=today_totals(); rem=remaining(p,totals)
            return render_template("meal.html",analysis=None,error=str(e),p=p,totals=totals,remaining=rem,today_progress=daily_nutrition_progress(p,totals),target_meta=calculate_targets(p),form_data=form_data,usage=today_usage(),limit=limit,form_token=secrets.token_urlsafe(24))
        ...
        # 2.4.13: durable background queue. The web process stops here after
        # safely storing the compressed photos + form payload in PostgreSQL.
        payload={
            "text":text,"meal_type":meal_type,"timing":timing,
            "hunger_before":num("hunger_before"),"fullness_after":num("fullness_after"),
            "water_ml":num("water_ml"),"note":request.form.get("note","").strip(),
            "record_date":record_choice,"record_day":record_day.isoformat(),
            "is_backfill":bool(is_backfill)
        }
        try:
            job_id=enqueue_analysis_job(user_key(),submitted_token,payload,raws)
            raws.clear(); gc.collect()
            app.logger.info("ANALYSIS_JOB queued id=%s user=%s",job_id,user_key())
            flash("餐點已收到，AI 正在分析中。你可以先離開這個頁面，稍後回到『查看最近紀錄』看結果，不需要重複送出。")
            return redirect(url_for("week"))
        except Exception:
            app.logger.exception("Failed to enqueue meal analysis")
            c=db(); c.execute("DELETE FROM submission_tokens WHERE token=? AND user_key=?",(submitted_token,user_key())); c.commit(); c.close()
            error="目前無法加入分析佇列，請稍後再試。這次沒有建立餐點紀錄。"
            form_token=secrets.token_urlsafe(24)
    totals=today_totals(); rem=remaining(p,totals); today_progress=daily_nutrition_progress(p,totals)
    return render_template("meal.html",analysis=analysis,error=error,p=p,totals=totals,remaining=rem,today_progress=today_progress,target_meta=calculate_targets(p),form_data=form_data,usage=today_usage(),limit=int((membership() or {}).get("daily_limit") or 6),form_token=form_token,today_label=taipei_now().strftime("%m/%d"),yesterday_label=(taipei_now()-timedelta(days=1)).strftime("%m/%d"))
...
def current_program_day():
    m=membership()
    if not m or not m.get("program_starts_at"):
        return None
    try:
        dt=datetime.fromisoformat(str(m.get("program_starts_at")))
        if dt.tzinfo is None:
            dt=dt.replace(tzinfo=timezone.utc)
        start_day=dt.astimezone(TAIPEI_TZ).date()
        return (taipei_now().date()-start_day).days+1
    except Exception:
        return None

def progress_requirements(p=None):
    p=p or get_profile() or {}
    day=current_program_day()
    waist_due=bool(day and day>0 and day%7==0)
    required=["weight","hunger","sleep_hours","sleep_quality","bowel_status","fatigue_level","activity_note"]
    if str(p.get("sex") or "").lower()=="female":
        required.append("menstrual_status")
    if waist_due:
        required.append("waist")
    return {"required":required,"waist_due":waist_due,"program_day":day}

def today_checkin():
    start,end=taipei_day_utc_bounds()
    c=db(); r=c.execute("SELECT * FROM checkins WHERE user_key=? AND created_at>=? AND created_at<? ORDER BY created_at DESC LIMIT 1",(user_key(),start,end)).fetchone(); c.close()
    return dict(r) if r else None

def checkin_completion(p=None):
    req=progress_requirements(p)
    row=today_checkin()
    labels={"weight":"體重","hunger":"飢餓感","sleep_hours":"睡眠時數","sleep_quality":"睡眠狀態","bowel_status":"排便狀態","menstrual_status":"生理期狀態","fatigue_level":"精神／疲勞","activity_note":"活動狀況","waist":"腰圍"}
    missing=[]
    if not row:
        missing=[labels[x] for x in req["required"]]
    else:
        for x in req["required"]:
            v=row.get(x)
            if v is None or (isinstance(v,str) and not v.strip()):
                missing.append(labels[x])
    return {**req,"done":bool(row) and not missing,"missing":missing,"row":row}

def adaptive_review(userkey, checkpoint=14):
    """Trend-based coaching summary. Never diagnoses endocrine disease and never auto-cuts calories."""
    c=db()
    p=c.execute("SELECT * FROM profiles WHERE user_key=?",(userkey,)).fetchone()
    if not p: c.close(); return None
    since=(utc_now_naive()-timedelta(days=checkpoint)).isoformat(timespec="seconds")
    meals=c.execute("SELECT * FROM meals WHERE user_key=? AND deleted_at IS NULL AND created_at>=? ORDER BY created_at",(userkey,since)).fetchall()
    checks=c.execute("SELECT * FROM checkins WHERE user_key=? AND created_at>=? ORDER BY created_at",(userkey,since)).fetchall()
    fasts=c.execute("SELECT * FROM fasting_sessions WHERE user_key=? AND created_at>=? ORDER BY created_at",(userkey,since)).fetchall()
    c.close()
    meal_dayset={str(r['created_at'])[:10] for r in meals}
    female=str(p['sex'] or '').lower()=='female'
    body_dayset=set()
    for r in checks:
        required_ok=(r['weight'] is not None and r['hunger'] is not None and r['sleep_hours'] is not None and bool(r['sleep_quality']) and bool(r['bowel_status']) and r['fatigue_level'] is not None and bool(r['activity_note']))
        if female:
            required_ok=required_ok and bool(r['menstrual_status'])
        if required_ok:
            body_dayset.add(str(r['created_at'])[:10])
    # A day counts as complete for adaptive review only when both food and mandatory body-response data exist.
    complete_dayset=meal_dayset & body_dayset
    logged_days=len(complete_dayset)
    adherence=min(100,round(logged_days/max(1,checkpoint)*100))
    avg_kcal=(sum(float(r['calories'] or 0) for r in meals)/max(1,logged_days)) if meals else 0
    avg_pro=(sum(float(r['protein_g'] or 0) for r in meals)/max(1,logged_days)) if meals else 0
    avg_veg=(sum(float(r['veg_fists'] or 0) for r in meals)/max(1,logged_days)) if meals else 0
    weights=[float(r['weight']) for r in checks if r['weight'] is not None]
    waists=[float(r['waist']) for r in checks if r['waist'] is not None]
    bfs=[float(r['body_fat']) for r in checks if r['body_fat'] is not None]
    def delta(xs): return round(xs[-1]-xs[0],1) if len(xs)>=2 else None
    wd, wad, bfd=delta(weights),delta(waists),delta(bfs)
    target=float(p['daily_calories'] or 0); pro_target=float(p['daily_protein'] or 0)
    kcal_ratio=avg_kcal/target if target and avg_kcal else None
    pro_ratio=avg_pro/pro_target if pro_target and avg_pro else None
    enough_data=logged_days >= max(4, round(checkpoint*0.6)) and len(weights)>=2
    improving=(wd is not None and wd <= -0.3) or (wad is not None and wad <= -1.0) or (bfd is not None and bfd <= -0.5)
    if not enough_data:
        status="資料不足"
        headline="先累積足夠資料，不急著改方案"
        action="維持目前設定，優先把餐點與體態紀錄補完整；資料不足時不自動降低熱量。"
    elif improving:
        status="有反應"
        headline="目前已有體態變化，先維持方案"
        action="暫不降低熱量；繼續觀察 7 日平均體重、腰圍與執行穩定度。"
    elif adherence < 70 or (kcal_ratio is not None and kcal_ratio > 1.12):
        status="先改善執行"
        headline="目前先處理執行穩定度，不急著砍熱量"
        action="下一階段優先提高紀錄完整度、外食估算與蛋白質／蔬菜達成度，再判斷是否需要調整目標。"
    elif checkpoint >= 21:
        status="建議人工檢視"
        headline="執行度不差，但體態反應低於預期"
        action="先由顧問檢查照片、份量、活動與生活狀態；若連續 3–4 週高度執行仍無合理變化，再考慮小幅調整方案或轉介專業評估。系統不自行診斷荷爾蒙問題。"
    else:
        status="持續觀察"
        headline="目前還不適合只靠短期體重判斷"
        action="維持目前方案到下一個檢視點，同時追蹤腰圍、體脂、睡眠、排便、生理期與活動狀態。"
    return {"checkpoint":checkpoint,"logged_days":logged_days,"adherence":adherence,"avg_kcal":round(avg_kcal),"avg_pro":round(avg_pro),"avg_veg":round(avg_veg,1),"weight_delta":wd,"waist_delta":wad,"bodyfat_delta":bfd,"checkins":len(checks),"fasting_sessions":len(fasts),"status":status,"headline":headline,"action":action,"protein_ratio":pro_ratio}
...
def progress_tracking():
    ...
    error=None
    if request.method=="POST":
        keys=["weight","body_fat","waist","hunger","sleep_hours","sleep_quality","bowel_status","menstrual_status","fatigue_level","activity_note","note"]
        vals={k:(request.form.get(k) or "").strip() for k in keys}
        labels={"weight":"體重","hunger":"飢餓感","sleep_hours":"睡眠時數","sleep_quality":"睡眠狀態","bowel_status":"排便狀態","menstrual_status":"生理期狀態","fatigue_level":"精神／疲勞","activity_note":"活動狀況","waist":"腰圍"}
        missing=[labels[k] for k in req["required"] if not vals.get(k)]
        form_data=vals
        # numeric validity
        w=_safe_float(vals['weight']); sh=_safe_float(vals['sleep_hours']); wa=_safe_float(vals['waist']); bf=_safe_float(vals['body_fat'])
        numeric_bad=[]
        if vals['weight'] and (w is None or w<25 or w>300): numeric_bad.append("體重")
        if vals['sleep_hours'] and (sh is None or sh<0 or sh>24): numeric_bad.append("睡眠時數")
        if vals['waist'] and (wa is None or wa<30 or wa>250): numeric_bad.append("腰圍")
        if vals['body_fat'] and (bf is None or bf<2 or bf>75): numeric_bad.append("體脂率")
        try: hunger=int(vals['hunger']) if vals['hunger'] else None
        except: hunger=None; numeric_bad.append("飢餓感")
        try: fatigue=int(vals['fatigue_level']) if vals['fatigue_level'] else None
        except: fatigue=None; numeric_bad.append("精神／疲勞")
        if hunger is not None and not 1<=hunger<=10: numeric_bad.append("飢餓感")
        if fatigue is not None and not 1<=fatigue<=5: numeric_bad.append("精神／疲勞")
        if missing or numeric_bad:
            parts=[]
            if missing: parts.append("尚未完成："+"、".join(missing))
            if numeric_bad: parts.append("請檢查數值："+"、".join(dict.fromkeys(numeric_bad)))
            error="；".join(parts)+"。完成必填項目後才能儲存今天紀錄。"
        else:
            start_day,end_day=taipei_day_utc_bounds()
            c=db()
            # one complete body-response record per Taiwan calendar day
            c.execute("DELETE FROM checkins WHERE user_key=? AND created_at>=? AND created_at<?",(user_key(),start_day,end_day))
            c.execute("INSERT INTO checkins(user_key,weight,body_fat,waist,hunger,sleep_hours,sleep_quality,bowel_status,menstrual_status,fatigue_level,activity_note,note,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(
              user_key(),w,bf,wa,hunger,sh,vals['sleep_quality'],vals['bowel_status'],vals['menstrual_status'] or ("不適用" if str(p.get('sex') or '').lower()!='female' else ""),fatigue,vals['activity_note'],vals['note'],db_now_iso()))
            c.commit(); c.close(); flash("今天的身體反應已完整記錄。")
            return redirect(url_for("progress_tracking"))
    c=db(); rows=c.execute("SELECT * FROM checkins WHERE user_key=? ORDER BY created_at DESC LIMIT 60",(user_key(),)).fetchall(); c.close()
    reviews=[adaptive_review(user_key(),d) for d in (7,14,21,28)]
    completion=checkin_completion(p)
    return render_template("progress.html",p=p,rows=rows,reviews=reviews,requirements=req,completion=completion,error=error,form_data=form_data)

@app.route("/progress/<int:cid>/delete",methods=["POST"])
def delete_progress(cid):
    c=db(); c.execute("DELETE FROM checkins WHERE id=? AND user_key=?",(cid,user_key())); c.commit(); c.close()
    return redirect(url_for("progress_tracking"))
...
def week():
    ...
    c.close()
    water_target=float(p["daily_water_ml"] or 0) if p else 0
    meal_days=group_meals_by_taipei_day(rows,water_rows,water_target)
    jobs=pending_analysis_jobs(user_key())
    return render_template("week.html",rows=rows,water_rows=water_rows,meal_days=meal_days,p=p,photos=photos_by_meal(rows),analysis_jobs=jobs)
...
def photos_by_meal(meals):
    """Return photo metadata only; never bulk-load Base64 image bodies.

    History/admin pages used to SELECT * for every photo, causing all Base64
    images for 7/14/21/30 days to coexist in one Python response. The browser
    now lazy-loads each photo through meal_photo(), one image per request.
    """
    result={}
    ids=[int(m["id"]) for m in (meals or []) if m.get("id") is not None]
    if not ids:return result
    placeholders=",".join("?" for _ in ids)
    c=db()
    rows=c.execute(f"SELECT id,meal_id,created_at FROM meal_photos WHERE meal_id IN ({placeholders}) ORDER BY meal_id,id",tuple(ids)).fetchall()
    c.close()
    for ph in rows:
        result.setdefault(ph["meal_id"],[]).append(ph)
    return result

@app.route("/meal-photo/<int:photo_id>")
def meal_photo(photo_id):
    c=db(); ph=c.execute("SELECT id,user_key,data_url FROM meal_photos WHERE id=?",(photo_id,)).fetchone(); c.close()
    if not ph:return "Not found",404
    allowed=False
    u=current_line_user()
    if u and ph["user_key"]=="line-"+u.get("userId",""):
        allowed=True
    if not allowed and admin_ok():
        allowed=True
    if not allowed:
        con=consultant_current()
        if con and str(ph["user_key"]).startswith("line-"):
            allowed=can_view_client(str(ph["user_key"])[5:])
    if not allowed:return "Unauthorized",401
    data_url=ph["data_url"] or ""
    try:
...
def admin():
    ...
    consultants=c.execute("SELECT * FROM consultants ORDER BY active DESC,name").fetchall(); c.close()
    state=(request.args.get("state") or "active").strip().lower()
    if state not in ("active","unopened","expired","inactive","all"): state="active"
    row_dicts=[dict(r) for r in rows]
    for r in row_dicts: r["effective_state"]=member_admin_state(r)
    # Newly LINE-authenticated members are surfaced separately on every admin
    # page so TDJ does not miss someone just because the current filter is
    # "有效".  The unopened tab remains available for a focused list.
    pending_rows=[r for r in row_dicts if r["effective_state"]=="unopened"]
    filtered_rows=row_dicts if state=="all" else [r for r in row_dicts if r["effective_state"]==state]
    counts={k:sum(1 for r in row_dicts if r["effective_state"]==k) for k in ("active","unopened","expired","inactive")}
    counts["all"]=len(row_dicts)

    # Total-admin student list is grouped by ownership. Keep TDJ Direct first,
    # then every consultant account (including consultants with zero students).
    # Counts follow the currently selected member-state filter.
    direct_rows=[r for r in filtered_rows if not r.get("consultant_id")]
    student_groups=[{
        "key":"tdj-direct",
        "label":"TDJ 直營",
        "rows":direct_rows,
    ...
    for con in consultants:
        con_rows=[r for r in filtered_rows if r.get("consultant_id")==con["id"]]
        student_groups.append({
            "key":f"consultant-{con['id']}",
            "label":con["name"],
            "rows":con_rows,
            "count":len(con_rows),
            "is_direct":False,
    ...
    return render_template("admin.html",rows=filtered_rows,pending_rows=pending_rows,consultants=consultants,student_groups=student_groups,token=request.args.get("token"),state=state,counts=counts)
...
@app.route("/admin/consultant/<int:consultant_id>/status",methods=["POST"])
def admin_consultant_status(consultant_id):
    if not admin_ok(): return "Unauthorized",401
    active=1 if request.form.get("action")=="activate" else 0
    c=db(); c.execute("UPDATE consultants SET active=? WHERE id=?",(active,consultant_id)); c.commit(); c.close()
    return redirect(url_for("admin",token=request.form.get("token"),state=request.form.get("state","active")))

@app.route("/admin/consultant/<int:consultant_id>/delete",methods=["POST"])
def admin_consultant_delete(consultant_id):
    if not admin_ok(): return "Unauthorized",401
    c=db()
    c.execute("UPDATE memberships SET consultant_id=NULL,updated_at=? WHERE consultant_id=?",(db_now_iso(),consultant_id))
    c.execute("UPDATE consultant_notes SET consultant_id=NULL WHERE consultant_id=?",(consultant_id,))
    c.execute("DELETE FROM consultants WHERE id=?",(consultant_id,)); c.commit(); c.close()
    return redirect(url_for("admin",token=request.form.get("token"),state=request.form.get("state","active")))

@app.route("/admin/member/<uid>/status",methods=["POST"])
def admin_member_status(uid):
    if not admin_ok(): return "Unauthorized",401
    action=request.form.get("action")
    c=db()
    if action=="deactivate":
        c.execute("UPDATE memberships SET status='inactive',updated_at=? WHERE line_user_id=?",(db_now_iso(),uid))
    c.commit(); c.close()
    return redirect(url_for("admin",token=request.form.get("token"),state=request.form.get("state","active")))

@app.route("/admin/member/<uid>/delete",methods=["POST"])
def admin_member_delete(uid):
    if not admin_ok(): return "Unauthorized",401
    uk="line-"+uid; c=db()
    # Permanent deletion: remove user-owned records first, then membership/profile.
    c.execute("DELETE FROM meal_photos WHERE user_key=?",(uk,))
    c.execute("DELETE FROM meals WHERE user_key=?",(uk,))
    c.execute("DELETE FROM hydration_logs WHERE user_key=?",(uk,))
    c.execute("DELETE FROM checkins WHERE user_key=?",(uk,))
    c.execute("DELETE FROM submission_tokens WHERE user_key=?",(uk,))
    c.execute("DELETE FROM fasting_sessions WHERE user_key=?",(uk,))
    c.execute("DELETE FROM fasting_settings WHERE user_key=?",(uk,))
    c.execute("DELETE FROM adaptive_reviews WHERE user_key=?",(uk,))
    c.execute("DELETE FROM consultant_notes WHERE line_user_id=?",(uid,))
    c.execute("DELETE FROM profiles WHERE line_user_id=? OR user_key=?",(uid,uk))
    c.execute("DELETE FROM memberships WHERE line_user_id=?",(uid,))
    c.commit(); c.close()
    return redirect(url_for("admin",token=request.form.get("token"),state=request.form.get("state","active")))
...
def group_meals_by_taipei_day(meals, water_logs=None, water_target=0):
    """Group meal and hydration rows into Taiwan calendar days for coach/admin review."""
    by_day={}; today_key=taipei_now().date().isoformat(); water_logs=water_logs or []

    def ensure_day(key, created_at):
        if key not in by_day:
            by_day[key]={
                "key":key,
                "label":taipei_time(created_at, "%Y/%m/%d"),
                ...
                "water_entries":0,
                "water_logs":[],
                "water_target":round(float(water_target or 0)),
                "water_pct":None,
            }
        return by_day[key]

    for r in meals:
        key=taipei_time(r["created_at"], "%Y-%m-%d")
        g=ensure_day(key, r["created_at"]); g["meals"].append(r); g["count"]+=1
        g["calories"]+=float(r["calories"] or 0); g["protein"]+=float(r["protein_g"] or 0)
        g["carbs"]+=float(r["carbs_g"] or 0); g["fat"]+=float(r["fat_g"] or 0); g["veg"]+=float(r["veg_fists"] or 0)
        # Meal-adjacent water is part of the same day's hydration total.
        g["water_ml"]+=int(r["water_ml"] or 0)

    for w in water_logs:
        key=taipei_time(w["created_at"], "%Y-%m-%d")
        g=ensure_day(key, w["created_at"]); g["water_ml"]+=int(w["amount_ml"] or 0); g["water_entries"]+=1; g["water_logs"].append(w)

    for g in by_day.values():
        if g["water_target"]:
            g["water_pct"]=round(g["water_ml"]/g["water_target"]*100)

    return sorted(by_day.values(), key=lambda g:g["key"], reverse=True)
...
def client_detail(uid):
    ...
    water_params=["line-"+uid]; water_sql="SELECT * FROM hydration_logs WHERE user_key=?"
    if days_i>0:
        water_sql+=" AND created_at>=?"; water_params.append(since)
    water_sql+=" ORDER BY created_at DESC LIMIT 500"
    water_logs=c.execute(water_sql,tuple(water_params)).fetchall()
    # Admin hydration summary: today actual intake + recent calendar-day average/adherence.
    water_target=float(p["daily_water_ml"] or 0) if p else 0
    today_start,today_end=taipei_day_utc_bounds()
    today_water_row=c.execute("SELECT COALESCE(SUM(amount_ml),0) AS total FROM hydration_logs WHERE user_key=? AND created_at>=? AND created_at<?",("line-"+uid,today_start,today_end)).fetchone()
    today_meal_water_row=c.execute("SELECT COALESCE(SUM(water_ml),0) AS total FROM meals WHERE user_key=? AND created_at>=? AND created_at<? AND deleted_at IS NULL",("line-"+uid,today_start,today_end)).fetchone()
    today_water=int(today_water_row["total"] or 0)+int(today_meal_water_row["total"] or 0)
    water_window=days_i if days_i in (7,14,21,30) else 30
    window_start_day=taipei_now().date()-timedelta(days=water_window-1)
    window_start,_=taipei_day_utc_bounds(window_start_day)
    window_rows=c.execute("SELECT amount_ml,created_at FROM hydration_logs WHERE user_key=? AND created_at>=? AND created_at<?",("line-"+uid,window_start,today_end)).fetchall()
    window_meal_water_rows=c.execute("SELECT water_ml,created_at FROM meals WHERE user_key=? AND created_at>=? AND created_at<? AND deleted_at IS NULL",("line-"+uid,window_start,today_end)).fetchall()
    water_by_day={}
    for wr in window_rows:
        day_key=taipei_time(wr["created_at"],"%Y-%m-%d")
        water_by_day[day_key]=water_by_day.get(day_key,0)+int(wr["amount_ml"] or 0)
    for mr in window_meal_water_rows:
        if not int(mr["water_ml"] or 0):
            continue
        day_key=taipei_time(mr["created_at"],"%Y-%m-%d")
        water_by_day[day_key]=water_by_day.get(day_key,0)+int(mr["water_ml"] or 0)
    avg_water=round(sum(water_by_day.values())/water_window) if water_window else 0
    today_water_pct=round(today_water/water_target*100) if water_target else None
    avg_water_pct=round(avg_water/water_target*100) if water_target else None
    hydration_summary={"today_ml":today_water,"target_ml":round(water_target),"today_pct":today_water_pct,"window_days":water_window,"avg_ml":avg_water,"avg_pct":avg_water_pct,"logged_days":len(water_by_day)}
    checkins=c.execute("SELECT * FROM checkins WHERE user_key=? ORDER BY created_at DESC LIMIT 100",("line-"+uid,)).fetchall()
    ...
    review=adaptive_review("line-"+uid, days_i if days_i in (7,14,21,28) else 28)
    meal_days=group_meals_by_taipei_day(meals, water_logs, water_target)
    return render_template("client_detail.html",p=p,m=m,meals=meals,meal_days=meal_days,water_logs=water_logs,hydration_summary=hydration_summary,checkins=checkins,adaptive_review=review,photos=photos_by_meal(meals),notes=notes,days=days_i,token=request.args.get("token"),is_admin=admin_ok(),consultant=consultant_current())
...
@app.route("/health")
def health(): return {"status":"ok","version":HEALTH_VERSION,"database":"postgres" if USE_POSTGRES else "sqlite"}
