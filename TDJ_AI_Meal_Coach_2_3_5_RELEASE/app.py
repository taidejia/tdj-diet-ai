...
def client_detail(uid):
    ...
    return render_template("client_detail.html",p=p,m=m,meals=meals,water_logs=water_logs,checkins=checkins,adaptive_review=review,photos=photos_by_meal(meals),notes=notes,days=days_i,token=request.args.get("token"),is_admin=admin_ok(),consultant=consultant_current())
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-2.3.5","database":"postgres" if USE_POSTGRES else "sqlite"}
