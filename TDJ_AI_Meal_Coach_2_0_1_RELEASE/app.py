...
def next_meal_from_daily_progress(progress, remaining, meal_type=None, timing="before", fullness_after=None):
    """Client-facing next-meal coaching with concrete Taiwan eating-out options."""
    try:
    ...
    # Human-readable target. Remaining numbers are caps/reference, not a command to fill them.
    target=[]
    if protein_need:
        target.append("蛋白質約 25–35g")
    if veg_need:
        target.append("蔬菜 1–2拳")
    if carbs_high:
        target.append("主食抓小份")
    elif c >= 30:
        target.append("主食約半碗–1碗，依飢餓感調整")
    if fat_high:
        target.append("烹調以清蒸、滷、烤、燙為主")
    if target:
        parts.append("下一餐重點："+"、".join(target)+"。")
    else:
        parts.append("下一餐照正常份量吃即可，優先維持一份蛋白質＋蔬菜，主食依飢餓感安排。")

    # Dynamic eating-out guide. Keep choices concrete and familiar in Taiwan.
    guides=[]
    rice = "飯先抓半碗左右" if carbs_high else "飯半碗到1碗，依餓不餓調整"
    if fat_high:
        guides.append(f"便當／自助餐：選滷雞腿去皮、烤雞、白肉魚或瘦肉1份＋2樣青菜＋{rice}；炸排骨、肥肉、勾芡與濃醬先跳過。")
        guides.append("超商：雞胸肉或茶葉蛋2顆＋沙拉／關東煮蔬菜；若還餓再加小地瓜或飯糰半份，先不配堅果與高脂醬料。")
        guides.append("滷味／鹽水雞：雞肉、豆腐或蛋擇2種＋2–3種蔬菜；麵、冬粉擇一小份，醬少、不要額外淋油。")
    elif protein_need and veg_need:
        guides.append(f"便當／自助餐：雞腿、雞胸、魚或瘦肉1份＋2樣青菜＋{rice}。")
        guides.append("超商：雞胸肉＋茶葉蛋＋沙拉；還餓可加地瓜或飯糰，飲料選無糖。")
        guides.append("滷味／鹽水雞：雞肉或瘦肉＋蛋／豆干＋2–3種蔬菜，主食可選一份冬粉或少量麵。")
        guides.append("麵店：乾麵／湯麵小碗＋滷蛋或豆干＋一盤燙青菜；如果肉量很少，再補一份瘦肉或無糖豆漿。")
    elif protein_need:
        guides.append(f"便當／自助餐：肉或魚1份＋至少1樣青菜＋{rice}。")
        guides.append("早餐店：里肌蛋吐司／蛋餅加蛋＋無糖豆漿；薯餅、雞塊等炸物不用一起點。")
        guides.append("超商：雞胸肉＋茶葉蛋或無糖豆漿；主食依飢餓感加地瓜或飯糰。")
    elif veg_need:
        guides.append(f"便當／自助餐：原本主菜照吃，青菜直接選2樣＋{rice}。")
        guides.append("火鍋：先拿2拳以上蔬菜＋一份肉／魚／豆腐；丸餃少量，主食只選飯、麵、冬粉其中一種。")
        guides.append("麵店：原本麵食可吃，但另外加一盤燙青菜，並搭滷蛋、豆干或瘦肉。")
    else:
        guides.append(f"便當／自助餐：一份肉或魚＋1–2樣青菜＋{rice}。")
        guides.append("超商：蛋白質主食＋沙拉／蔬菜，再依飢餓感決定要不要加地瓜或飯糰。")
        guides.append("火鍋：肉／魚一份＋蔬菜至少1–2拳，主食選一種即可。")

    parts.append("外食可以這樣選：" + " ".join(f"{i+1}) {g}" for i,g in enumerate(guides[:4])))

    if 0 < cal < 250:
        parts.append(f"今天約還有 {cal} kcal 的熱量參考空間；如果下一次只是一點餓，改成小份點心即可，不必再吃完整一餐。")
    parts.append("上面的份量是實用參考，不需要為了把剩餘數字吃滿而硬吃。")
    return "".join(parts)
...
def admin():
    ...
    rows=c.execute("""SELECT m.*,p.name,p.weight,p.goal,
      ...
      FROM memberships m LEFT JOIN profiles p ON p.line_user_id=m.line_user_id ORDER BY m.updated_at DESC""",(start_utc,end_utc)).fetchall(); c.close()
...
def admin_member(uid):
    if not admin_ok(): return "Unauthorized",401
    days=int(request.form.get("days","30")); now=utc_now_naive(); c=db()
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-2.0.1","database":"postgres" if USE_POSTGRES else "sqlite"}
