...
def init_db():
    ...
    if USE_POSTGRES:
        ...
        CREATE TABLE IF NOT EXISTS checkins(
          ...
          menstrual_status TEXT, activity_note TEXT, water INTEGER, note TEXT, created_at TEXT
        );
    ...
    CREATE TABLE IF NOT EXISTS checkins(
      ...
      activity_note TEXT, water INTEGER, note TEXT, created_at TEXT
    );
    """)
    ...
    for col,typ in [("waist","REAL"),("hunger","INTEGER"),("sleep_hours","REAL"),("sleep_quality","TEXT"),("bowel_status","TEXT"),("menstrual_status","TEXT"),("activity_note","TEXT")]:
...
def home():
    ...
    return render_template("home.html",profile=p,line_user=current_line_user(),member=membership(),active=active,totals=totals,progress=progress,remaining=rem,target_meta=calculate_targets(p) if p else None,fasting_state=fasting_state() if active and p else {"enabled":False})
...
def adaptive_review(userkey, checkpoint=14):
    ...
    c.close()
    dayset={str(r['created_at'])[:10] for r in meals}; logged_days=len(dayset)
...
def progress_tracking():
    ...
    if not p: return redirect(url_for("assessment"))
    if request.method=="POST":
        vals={k:request.form.get(k) for k in ["weight","body_fat","waist","hunger","sleep_hours","sleep_quality","bowel_status","menstrual_status","activity_note","note"]}
        c=db(); c.execute("INSERT INTO checkins(user_key,weight,body_fat,waist,hunger,sleep_hours,sleep_quality,bowel_status,menstrual_status,activity_note,note,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",(
          user_key(),_safe_float(vals['weight']),_safe_float(vals['body_fat']),_safe_float(vals['waist']),int(vals['hunger']) if vals['hunger'] else None,_safe_float(vals['sleep_hours']),vals['sleep_quality'],vals['bowel_status'],vals['menstrual_status'],vals['activity_note'],vals['note'],db_now_iso()))
        c.commit(); c.close(); flash("今天的身體反應已記錄。")
    ...
    return render_template("progress.html",p=p,rows=rows,reviews=reviews)
...
@app.route("/health")
def health(): return {"status":"ok","version":"meal-coach-2.3.1","database":"postgres" if USE_POSTGRES else "sqlite"}
