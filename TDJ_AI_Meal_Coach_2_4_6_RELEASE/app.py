...
def group_meals_by_taipei_day(meals):
    """Group newest-first meal rows into Taiwan calendar days for coach/admin review."""
    groups=[]; by_day={}; today_key=taipei_now().date().isoformat()
    for r in meals:
        key=taipei_time(r["created_at"], "%Y-%m-%d")
        if key not in by_day:
            g={
                "key":key,
                "label":taipei_time(r["created_at"], "%Y/%m/%d"),
            ...
            by_day[key]=g; groups.append(g)
        g=by_day[key]; g["meals"].append(r); g["count"]+=1
...
def client_detail(uid):
    ...
    water_logs=c.execute("SELECT * FROM hydration_logs WHERE user_key=? ORDER BY created_at DESC LIMIT 500",("line-"+uid,)).fetchall()
    ...
    meal_days=group_meals_by_taipei_day(meals)
