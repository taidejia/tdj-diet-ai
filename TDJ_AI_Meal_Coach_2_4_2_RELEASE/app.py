...
def admin():
    ...
    return render_template("admin.html",rows=filtered_rows,pending_rows=pending_rows,consultants=consultants,token=request.args.get("token"),state=state,counts=counts)
...
def client_detail(uid):
    ...
    return render_template("client_detail.html",p=p,m=m,meals=meals,meal_days=meal_days,water_logs=water_logs,checkins=checkins,adaptive_review=review,photos=photos_by_meal(meals),notes=notes,days=days_i,token=request.args.get("token"),is_admin=admin_ok(),consultant=consultant_current())
