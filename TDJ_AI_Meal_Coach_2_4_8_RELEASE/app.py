...
def _single_meal_statuses(data):
    ...
    data["fat_status"]="脂肪較高" if fat>=18 else ("脂肪中等" if fat>=8 else "脂肪較低")
...
def meal():
    ...
    if request.method=="POST":
        ...
        if analysis:
            ...
            summary="\n".join(history_parts)
            meal_source=("photo" if raw else "text") + ("_backfill" if is_backfill else "")
            recorded_at=meal_recorded_at(record_day)
            ...
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(user_key(),meal_type,meal_source,text,summary,recorded_at,num("hunger_before"),num("fullness_after"),num("water_ml"),request.form.get("note","").strip(),analysis.get("calories"),analysis.get("protein_g"),analysis.get("carbs_g"),analysis.get("fat_g"),analysis.get("veg_fists"),analysis.get("estimate_note")))
            ...
            for b in raws[:5]:
                ...
                c.execute("INSERT INTO meal_photos(meal_id,user_key,data_url,created_at) VALUES(?,?,?,?)",(meal_id,user_key(),data_url,recorded_at))
            c.commit(); c.close()
...
def week():
    ...
    return render_template("week.html",rows=rows,water_rows=water_rows,meal_days=meal_days,p=p,photos=photos_by_meal(rows))
