...
def member_admin_state(row):
    """Admin-facing effective state: active, expired, or inactive."""
    try:
        status=(row.get("status") if hasattr(row,"get") else row["status"]) or "inactive"
        expires=(row.get("expires_at") if hasattr(row,"get") else row["expires_at"])
    except Exception:
        return "inactive"
    if status != "active":
        return "inactive"
...
def admin():
    ...
    if state not in ("active","expired","inactive","all"): state="active"
    ...
    counts={k:sum(1 for r in row_dicts if r["effective_state"]==k) for k in ("active","expired","inactive")}
    counts["all"]=len(row_dicts)
    return render_template("admin.html",rows=filtered_rows,consultants=consultants,token=request.args.get("token"),state=state,counts=counts)
